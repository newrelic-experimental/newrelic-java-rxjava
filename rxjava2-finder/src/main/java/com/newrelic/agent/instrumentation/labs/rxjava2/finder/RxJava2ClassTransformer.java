...
public class RxJava2ClassTransformer implements ContextClassTransformer {
	...
	public RxJava2ClassTransformer(InstrumentationContextManager mgr) {
		contextManager = mgr;
	}
		...
		for (Method method : match.getMethods()) {
			for (ClassAndMethodMatcher matcher : match.getClassMatches().keySet()) {
				if (matcher.getMethodMatcher().matches(MethodMatcher.UNSPECIFIED_ACCESS, method.getName(),
					...
					context.putTraceAnnotation(method, TraceDetailsBuilder.newBuilder().setTracerFactoryName(RxJava2PreMain.TRACER_FACTORY)
