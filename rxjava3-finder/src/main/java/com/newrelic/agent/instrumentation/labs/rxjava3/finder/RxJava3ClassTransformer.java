...
public class RxJava3ClassTransformer implements ContextClassTransformer {
	...
	public RxJava3ClassTransformer(InstrumentationContextManager mgr) {
		contextManager = mgr;
	}
		...
		for (Method method : match.getMethods()) {
			for (ClassAndMethodMatcher matcher : match.getClassMatches().keySet()) {
				if (matcher.getMethodMatcher().matches(MethodMatcher.UNSPECIFIED_ACCESS, method.getName(),
					...
					context.putTraceAnnotation(method, TraceDetailsBuilder.newBuilder().setTracerFactoryName(RxJava3PreMain.TRACER_FACTORY)
